...
const PUSHBOX_OAUTH_SCOPE_SEND = 'https://identity.mozilla.com/apps/pushbox/send'

module.exports = (log, db, config, customs, push, pushbox, devices, oAuthMint) => {
  // Loads and compiles a json validator for the unencrypted payloads received
  ...
  return [
    ...
    },
    {
        ...
        validate: {
          payload: isA.alternatives().try(
            isA.object({
              ...
              _endpointAction: isA.string().valid('accountVerify', 'sendTab').optional(),
              excluded: isA.array().items(isA.string().length(32).regex(HEX_STRING)).optional(),
              payload: isA.object().when('_endpointAction', { is: 'accountVerify', then: isA.required() }),
              encryptedPayload: isA.string().regex(validators.URL_SAFE_BASE_64).when('_endpointAction', { is: 'sendTab', then: isA.required(), otherwise: isA.forbidden() }),
              TTL: isA.number().integer().min(0).optional()
            }).or('payload', 'encryptedPayload'),
            isA.object({
      ...
      handler (request, reply) {
        ...
        const usePushbox = body.hasOwnProperty('encryptedPayload')
        const payload = usePushbox ? body.encryptedPayload : body.payload
        const endpointAction = body._endpointAction || 'devicesNotify'

        if (! usePushbox && ! validatePushPayload(payload, endpointAction)) {
        ...
        return customs.checkAuthenticated(endpointAction, ip, uid)
          ...
          .then(devices => {
            ...
            function pushboxNotify() {
              const deviceIds = devices.map(device => device.id)
              return oAuthMint.getOAuthToken(sessionToken, PUSHBOX_OAUTH_SCOPE_SEND)
                .then(resp => {
                  const {access_token: oAuthToken} = resp
                  // At the moment, endpointAction can only be 'sendTab'.
                  return pushbox.sendTab(uid, deviceIds, payload, oAuthToken)
                })
                .then((responses) => {
                  return P.each(responses.entries(), ([deviceId, bodyOrError]) => {
                    if (! bodyOrError.hasOwnProperty('index')) {
                      // Server returned an error or we couldn't parse the response body.
                      catchPushError(bodyOrError)
                      return
                    }
                    const deviceToNotify = devices.find(d => d.id === deviceId)
                    return push.notifyPushboxUpdated(uid, deviceToNotify, 'sendtab',
                                                     endpointAction, bodyOrError.index)
                               .catch(catchPushError)
                  })
                })
            }

            function pushOnlyNotify() {
              const pushOptions = {
                data: payload
              }
              if (body.TTL) {
            ...
            return usePushbox ? pushboxNotify() : pushOnlyNotify()
          })
          .then(
            () => reply({}),
            reply
